package com.example.kechuanqi.materialdesign.view;
...
public class AppPreference extends BasePreference {
    ...
    public void updateStatusBarColor(@ColorInt int color) {
        ...
        editor.apply();
        for (int i = 0; i < themeChangedListenerLists.size(); i++) {
            ThemeChangeable themeChangeable = themeChangedListenerLists.get(i);
            themeChangeable.themeChange();
        }
    }

    /**
     * 不要直接调用该方法，除非你确定此时应用主题为【白天】，//应该使用 ColorUtils#getAccentColor(Context)}
     * 获取该值。
     *
     * @return 控件首选色
     */
    ...
    private ArrayList<ThemeChangeable> themeChangedListenerLists = new ArrayList<>();
    public void bindOnThemeChanged(ThemeChangeable onThemeChangedListener) {
        themeChangedListenerLists.add(onThemeChangedListener);
    }

    public void unbindOnThemeChanged(ThemeChangeable onThemeChangedListener) {
        boolean remove = themeChangedListenerLists.remove(onThemeChangedListener);
        Log.e("theme",getClass().getName()+"------- unbindOnThemeChanged remove="+remove);
    }
}
